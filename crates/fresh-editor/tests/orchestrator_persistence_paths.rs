...
/// Regression for issue #2056: launching `fresh .` in a project that
/// has previously-run orchestrator *worktree* sessions must open a
/// plain window rooted at the launch cwd — NOT resurrect a worktree
/// session.
///
/// Orchestrator sessions persist with `root` = the session's worktree
/// (a separate directory) but `project_path` = the parent project.
/// The active-window pick matched on `project_path`, so launching in
/// the project dir reopened a worktree session as the active window,
/// dragging the worktree's files/file-explorer/title into what the
/// user expected to be a clean project window (the screenshots in the
/// issue: title `… — anna-katharine-green_hand-and-ring`, file
/// explorer rooted at `ralestone`/`tmp` instead of the cwd).
#[test]
fn startup_in_project_does_not_resurrect_worktree_session() {
    ...
    // windows.json with an orchestrator worktree session that is the
    // globally last-used window (`active`). Its `root` is the worktree
    // but its `project_path` (both top-level and in plugin_state) is
    // the launch project — exactly what the orchestrator writes.
    let orch_dir = data_root.path().join("data").join("orchestrator");
    std::fs::create_dir_all(&orch_dir).unwrap();
    let persisted = serde_json::json!({
        "version": 2,
        ...
        "windows": [
            ...
            {
                "id": 2,
                "label": "anna-katharine-green_hand-and-ring",
                "root": worktree_canon,
                "project_path": project_canon,
                "plugin_state": {
                    "orchestrator": {
                        "project_path": project_canon.to_string_lossy(),
                    }
                }
            }
        ]
    ...
    let active_root = editor.active_window().root.clone();
    assert_eq!(
        active_root, project_canon,
        "launching in the project must open a window rooted at the launch cwd, \
         not the orchestrator worktree session at {worktree_canon:?}; got {active_root:?}"
    );
}
