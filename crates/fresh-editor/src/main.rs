...
fn real_main() -> AnyhowResult<()> {
    ...
    let (result, last_update_result) = loop {
        ...
        // the persisted windows.json + plugin state files (under
        // `data_dir/orchestrator/<encoded working dir>/`) and
        // builds the initial windows map / `plugin_global_state` directly
